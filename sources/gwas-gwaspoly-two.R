...
markersVennDiagrams <- function (listOfResultsFile, summaryTable, gwasModel, scoresType, outFile){
	...
	if (nrow (summaryTable) > 0) {
		...
		# Create Venn diagram
		nTools = length (x)
		msgmsg ("Plotting Venn diagram...", nTools)
		print (x)
		mainTitle = paste0(gwasModel, "-", scoresType)
		COLORS= c("red", "blue", "yellow", "green")[1:length (x)]
		v0 <- venn.diagram(x, height=2000, width=3000, alpha = 0.5, filename = NULL, # main=mainTitle,
							col = COLORS, cex=CEXLABELS, cat.cex=CEXTITLES, margin=0.0, fill = COLORS)
		...
		for (i in 1:length(overlaps)){
			pos = posOverlap [i]
			v0[[pos+nTools*2]]$label <- paste(overlaps[[i]], collapse = "\n")
		}
