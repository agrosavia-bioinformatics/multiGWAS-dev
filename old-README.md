...
# Data formats in MultiGWAS
MultiGWAS requires two mandatory files (the genotype and phenotype) and one optional file (information of markers). Currently, MultiGWAS accepts four formats for the genotype: "gwaspoly", "kmatrix", "vcf", and "fitpoly".
	- "gwaspoly" format: corresponds to a comma separated values (.csv) with the marker data for each sample codified in the "ACGT" format (e.g., AATT, CCGG, AAAT, GGCG), as the following example:
\small
...
```	
	 ...
	 | c2_21320 | 0     | 3810687  | TTTT     | TTTT        | TTTT        | TTTT           |
	 | c2_21318 | 0     | 3810936  | NA       | NA          | NA          | TCCC           |
```
	- "kmatrix" format: 
